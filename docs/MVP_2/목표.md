...
- [ ] `Symbol` VO 구현 (value 필드, null 불가, 대문자 고정)
- [ ] 동등성 비교 (`equals` / `hashCode`) 구현

### 1.2 OrderType (Enum)

- [ ] `OrderType` 추가: `LIMIT`, `MARKET`
- [ ] `isMarket()` 편의 메서드 구현
...
- [ ] `Order.createLimit(side, symbol, price, quantity)` 팩토리 메서드 구현
- [ ] `Order.createMarket(side, symbol, quantity)` 팩토리 메서드 구현
- [ ] 기존 생성자 private/package-private으로 제한
- [ ] `price` 필드 `@Nullable` 선언, public `getPrice()` 제거 또는 LIMIT 전용 명시
- [ ] `getLimitPriceOrThrow()` 구현 — MARKET 호출 시 예외
- [ ] `isMarket()` 편의 메서드 구현
- [ ] `symbol` 필드 추가
- [ ] `orderType` 필드 추가
