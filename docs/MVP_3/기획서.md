...
| `400` | `ORDER_TIF_INVALID`             | `ConstraintValidator` (DTO 레벨) | MARKET 주문에 tif 명시       |
| `400` | `MARKET_BUY_QUOTE_QTY_REQUIRED` | `ConstraintValidator` (DTO 레벨) | MARKET BUY에 quoteQty 누락 |
...
4. `PlaceOrderRequest.quoteQty` 필드 + MARKET `tif` 명시 검증 (`PlaceOrderValidator` 확장)
