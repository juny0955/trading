...
2. **MARKET BUY 완성**: `quoteQty` 기반 예산 한도 매칭
...
**수정 방향**: `Order.create()` 또는 Controller 레이어에서 입력을 검증하고 `ValidationException(400)`으로 변환한다.
...
| MARKET BUY 완성 | `quoteQty` 필드 + 예산 기반 매칭 |
...
| 번호 | 항목 | 결정 | 근거 |
|------|------|------|------|
| 1 | MARKET BUY 수량 표현 | `quoteQty` 필드 추가 | 업계 표준(Binance `quoteOrderQty`). Phase 4 잔고 홀드와 자연스럽게 연결 |
...
|-----------|---------|
| LIMIT | GTC, IOC, FOK |
| MARKET | `tif` 필드 명시 시 `ORDER_TIF_INVALID` 거부 |
...
Order (추가 필드)
  ...
  quoteQty: Long          // MARKET BUY 전용. 그 외 null
```

`quoteQty`는 MARKET BUY에서만 사용한다. Phase 3에서는 **매칭 한도(spending cap)** 역할만 한다.
...
## 7. MARKET BUY quoteQty 매칭 규칙

### 7.1 매칭 로직

MARKET BUY는 `quoteQty`(예산)를 기준으로 최대한 매수한다.
...
MARKET BUY에서 `Order.remaining`은 base qty 목표가 없으므로 의미가 없다.
매칭 중 예산 추적은 내부 변수 `remainingQuote`로 관리하며, `Order.remaining`은 0으로 고정한다.

조회 API 해석 규칙:
- MARKET BUY의 `remaining`은 항상 `0`으로 반환한다.
- 실제 진행/결과 해석은 `requestedQuoteQty`, `cumQuoteQty`, `cumBaseQty`로 수행한다.

### 7.3 종료 시 status 정의

MARKET BUY는 "예산 내 최선 매수"이므로 PARTIALLY_FILLED 상태를 두지 않는다.
예산이 일부만 소진됐더라도 1건이라도 체결됐으면 FILLED로 처리한다.

| 조건 | 최종 status |
|------|------------|
| 체결 0건 | `CANCELLED` |
| 체결 1건 이상 (예산 소진·유동성 소진 무관) | `FILLED` |

### 7.4 MARKET BUY 응답 필드

MARKET BUY의 `FILLED`는 "요청 quote 예산 내에서 1건 이상 체결됨" 의미다.
오해를 줄이기 위해 응답에 누적 체결 정보를 명시한다.

| 필드 | 의미 |
|------|------|
| `requestedQuoteQty` | 클라이언트가 요청한 예산 (`quoteQty`) |
...
- MARKET BUY: `quoteQty` 필수. `quantity` 무시
...
MARKET BUY 응답에는 `requestedQuoteQty`, `cumQuoteQty`, `cumBaseQty`를 포함한다.
...
| `400` | `ORDER_TIF_INVALID` | `ValidationException` | MARKET 주문에 tif 명시 |
| `400` | `MARKET_BUY_QUOTE_QTY_REQUIRED` | `ValidationException` | MARKET BUY에 quoteQty 누락 |
...
2. **B-2** `Order.create()` 또는 Controller 레이어 — `Side`/`OrderType` 잘못된 값 및 LIMIT `price=null` → `ValidationException(400)` 처리
...
5. `PlaceOrderRequest.tif` 필드 + `ValidationException` 연결
6. TIF 상태 전이 단위 테스트

### Phase 3-2. MARKET BUY quoteQty + 백프레셔

1. `Order.quoteQty` 필드 추가 (nullable)
2. `MatchingEngine.placeMarketOrder()` — MARKET BUY는 `quoteQty` 기반으로 분기
3. MARKET BUY 종료 status 규칙 구현 (7.3 정의 적용)
4. `PlaceOrderRequest.quoteQty` 필드 + MARKET `tif` 명시 검증 + `ValidationException` 연결
5. MARKET BUY 단위 테스트 (체결 0건 → CANCELLED / 체결 1건 이상 → FILLED)
...
| MARKET BUY 체결 성공 | 예산 소진·유동성 소진 무관, 1건 이상 → FILLED |
| MARKET BUY 유동성 없음 | 0건 체결 → CANCELLED |
| MARKET tif 명시 거부 | MARKET 요청에 tif 포함 → 400 ORDER_TIF_INVALID |
...
2. MARKET BUY quoteQty 종료 status(7.3 정의)대로 동작
