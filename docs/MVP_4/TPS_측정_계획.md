...
* lock 순서 고정 정책(asset 이름 기준 정렬)이 실제로 동작하는지 확인

입력 예:

* buy account 1개, sell account 1개
* symbol 1개 (`BTC`, quote = KRW, base = BTC)
...
* Persistence Layer에서 balance row lock 순서를 asset 이름 오름차순으로 고정
...
```sql
-- BUY 체결: buyer의 quote held 감소 == SUM(trade.price * trade.quantity)
-- SELL 체결: seller의 base held 감소 == SUM(trade.quantity)
